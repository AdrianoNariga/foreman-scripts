#!/bin/bash
name=foreman.local
smart_proxys="dhcp.local dns.local puppet.local"
...
prepare_instances(){
	...
	for i in $vms
	do
		ssh -t root@$i 'yum install vim git -y ; git clone https://github.com/narigacdo/home-jab.git ; bash /root/home-jab/foreman/set_fixed_ip.sh'
	done
}
